# Build Flowmatic thesis v3 PDF (v2 manuscript + supervisor intro revision + metadata fixes)
...
try {
  ...
  if (Test-Path "memoirthesis.pdf") {
    Copy-Item "memoirthesis.pdf" (Join-Path $root "memoirthesis-flowmatic-v3.pdf") -Force
    Write-Host "SUCCESS: $(Join-Path $root 'memoirthesis-flowmatic-v3.pdf')"
  }
