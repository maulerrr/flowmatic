...
## Realtime Lake + Export Semantics (implemented)

### Ingest path

1. Simulated or external WebSocket/HTTP source delivers a **chunk** (one or more JSON payloads).
2. Each payload is persisted as a `SensorEvent` in Postgres (short retention window).
3. When pipeline status is `ACTIVE`, the same event triggers medallion writes and optional core-unit inference.

### Medallion writes (S3/MinIO data lake)

| Mode | Behavior |
|------|----------|
| `append` (default) | Creates `stream.ndjson` under the hourly partition if missing; **appends one NDJSON line per event** per tier (`raw`, `cleaned`, `business`). |
| `object` | Writes one JSON object per event with a deterministic suffix key. |

Backfill uses deterministic `backfill-{eventTime}-{eventId}.json` keys so reruns are idempotent.

### Static export targets

| Adapter | Create vs append |
|---------|------------------|
| Postgres / MongoDB | `ifExists: append` creates collection/table if needed, then inserts rows. `replace` recreates/clears first. |
| JSON (continuous) | Appends NDJSON lines to a stable S3 key (`smart-city/{pipelineId}/{stage}/{targetId}.ndjson`). |
| JSON (manual) | Writes a snapshot file with a timestamp suffix. |

Continuous exports advance a cursor on `eventTime` and only run while the pipeline is `ACTIVE`.

### Pipeline runtime controls

- `POST /smart-city/pipelines/:id/start` — sets status `ACTIVE`, applies source interval, starts sources.
- `POST /smart-city/pipelines/:id/stop` — sets status `PAUSED`, stops sources, records last running source IDs.
- `POST /smart-city/pipelines/:id/resume` — restarts previously running sources.
- `PATCH /smart-city/pipelines/:id/runtime` — update `sourcePollIntervalMs`, `lakeWriteMode`, `exportCadenceSeconds`.

Demo script: `thesis/experiments/_run_minio_pipeline_demo.py`.
