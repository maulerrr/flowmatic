...
import { MoreVertical, Eye, Download, Trash2, CheckCircle, Clock, RefreshCw, AlertCircle } from 'lucide-vue-next'
...
<template>
	<div class="rounded-xl border border-border bg-card/40 backdrop-blur-sm hover:border-primary/40 transition-all group overflow-hidden">
		<!-- Card Header -->
		<div class="p-6 border-b border-border/30 bg-card/50 flex items-start justify-between">
			<div class="flex-1">
				<h3 class="font-semibold text-foreground mb-1 group-hover:text-primary transition">{{ run.sourceFileName }}</h3>
			...
			<div class="flex items-center gap-2">
				...
				>
					<component :is="getStatusConfig(run.status).icon" class="w-3.5 h-3.5" />
		...
		<div class="p-6 space-y-4">
			...
			<div class="grid grid-cols-2 gap-4">
				<div>
					...
					<p class="text-sm font-semibold text-foreground mt-1">{{ formatBytes(run.sourceFile?.fileSize || 0) }}</p>
				</div>
				<div>
			...
			<!-- Progress Bar -->
			<div v-if="run.status === 'processing'" class="space-y-2">
				<div class="flex items-center justify-between">
					<p class="text-xs text-foreground/60">Progress</p>
					<p class="text-xs font-semibold text-foreground">{{ run.rowsCleaned && run.rowsIngested ? Math.round((run.rowsCleaned / run.rowsIngested) * 100) : 0 }}%</p>
				</div>
		...
		<!-- Card Footer - Actions -->
		<div class="px-6 py-3 border-t border-border/30 bg-card/30 backdrop-blur-sm flex items-center justify-between">
