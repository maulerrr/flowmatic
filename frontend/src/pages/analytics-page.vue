...
import { BarChart3, CalendarDays, Download, PieChart, RefreshCw, TrendingUp } from 'lucide-vue-next';
		...
		<div class="max-w-7xl mx-auto px-6 py-8 md:px-8">
			...
			<div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-8">
				...
				<div class="flex gap-2">
					...
					<button
						class="flex items-center gap-2 px-4 py-2 rounded-lg bg-gradient-to-r from-primary to-secondary text-foreground font-medium transition hover:shadow-[var(--glow)]"
					>
			...
			<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
				<div
					...
					:key="metric.label"
					class="rounded-xl border border-border bg-card/70 backdrop-blur-md p-6 hover:border-primary/40 transition-all group"
				>
			...
			<div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
				<div
					...
					:key="chart.title"
					class="rounded-xl border border-border bg-card/70 backdrop-blur-md p-6 hover:border-primary/40 transition-all"
				>
					<div class="flex items-center gap-4 mb-6">
						<div
							:class="['w-12 h-12 rounded-lg bg-gradient-to-br flex items-center justify-center', chart.color]"
						>
							<component
								:is="chart.icon"
					...
					<div
						class="h-64 flex items-center justify-center bg-card/30 rounded-lg border border-border/30 overflow-hidden relative"
					>
						<div
							v-if="chartsData && chartsData[chart.key]"
							class="w-full h-full p-4"
						>
							<!-- Area Chart -->
							<div
								v-if="chart.type === 'area'"
								class="w-full h-full relative px-2 pt-4"
							>
								<svg
								...
								>
									...
									<path
										:d="getPaths(chartsData[chart.key].values).area"
										:fill="'url(#grad-' + chart.key + ')'"
										:class="chart.class"
										class="transition-all duration-500"
									/>
									<path
										:d="getPaths(chartsData[chart.key].values).line"
										fill="none"
										stroke="currentColor"
										stroke-width="3"
										:class="chart.class"
								...
								<div class="absolute inset-0 flex items-stretch mx-2 mt-4">
									...
									>
										<div
											class="absolute bottom-4 left-1/2 -translate-x-1/2 bg-popover text-popover-foreground text-xs px-2 py-1 rounded shadow-lg border border-border opacity-0 group-hover/bar:opacity-100 whitespace-nowrap z-20 pointer-events-none transition-all"
										>
							...
							<div
								...
								class="w-full h-full flex items-center justify-center gap-8"
							>
								<div
									class="relative w-48 h-48 rounded-full shadow-lg border-4 border-card"
									:style="getDonutStyle(chartsData[chart.key].values)"
								>
									<!-- Center hole -->
									<div
										class="absolute inset-4 bg-card rounded-full flex items-center justify-center flex-col shadow-inner"
									>
										<span
											class="text-3xl font-bold tracking-tight"
											>{{ chartsData[chart.key].values.reduce((a: any, b: any) => a + b, 0) }}</span
										>
										<span
								...
								<div class="flex flex-col gap-3 justify-center min-w-[120px]">
									<div
										v-for="(label, i) in chartsData[chart.key].labels"
										:key="label"
									...
									>
										...
										<div class="flex flex-col">
											...
											<span class="text-xs text-muted-foreground"
												>{{ chartsData[chart.key].values[i] }}
												({{ Math.round((chartsData[chart.key].values[i] / (chartsData[chart.key].values.reduce((a: any, b: any) => a + b, 0) || 1)) * 100)
							...
							<div
								v-else
								class="w-full h-full flex items-end gap-3 px-2 pb-2"
							>
								<div
									v-for="(val, idx) in chartsData[chart.key].values"
									:key="idx"
								...
								>
									<div
										:class="['w-full h-full rounded-t-md opacity-80 group-hover/bar:opacity-100 transition-all duration-300', chart.class.replace('text-', 'bg-')]"
									></div>
									<div
										class="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 bg-popover text-popover-foreground text-xs px-2 py-1 rounded shadow-lg border border-border opacity-0 group-hover/bar:opacity-100 whitespace-nowrap z-20 pointer-events-none transition-all translate-y-2 group-hover/bar:translate-y-0"
									>
						...
						<p
							v-else-if="loading"
							class="text-foreground/60 animate-pulse"
						>
							Loading data...
						</p>
						<p
			...
			<div
				v-if="summary"
				class="rounded-xl border border-border bg-card/70 backdrop-blur-md p-8"
			>
				...
				<div class="grid grid-cols-1 md:grid-cols-3 gap-6">
					<div class="pb-4 border-b border-border/20">
						...
						<p class="text-3xl font-bold text-foreground">
							{{ formatMs(summary.avgProcessingTime) }}
						</p>
					...
					<div class="pb-4 border-b border-border/20">
						...
						<p class="text-3xl font-bold text-foreground">
							{{ formatBytes(summary.totalDataProcessed) }}
						</p>
